"""
pages/stock_analysis_v3.py
Halaman Hasil Analisa Stock V3 — Logika baru dengan:
...
from utils import (
    ...
    classify_stock_vs_so_statistik,
...
# ── Render Utama ───────────────────────────────────────────────────────────────
def render():
    st.title("📈 Hasil Analisa Stock V3")
    st.info(f"""
    **Perbedaan dengan V1:**
    ...
    with st.expander("📖 Metode Klasifikasi Stok vs SO (klik untuk membaca)", expanded=False):
        st.markdown(STOK_SO_METHOD_DESCRIPTION)
    ...
    col1.date_input("Tanggal Awal (90 Hari Belakang)", value=default_start,
                    key="stock_v3_start", disabled=True)
    end_date = col2.date_input("Tanggal Akhir", value=default_end, key="stock_v3_end")

    if st.button("🚀 Jalankan Analisa Stock V3"):
        _run_analysis_v3(penjualan, produk_ref, df_stock, end_date)

    if st.session_state.get("stock_v3_result") is not None:
        _render_results_v3()


# ── Kalkulasi Utama ────────────────────────────────────────────────────────────
def _run_analysis_v3(penjualan, produk_ref, df_stock, end_date):
    with st.spinner("Melakukan perhitungan analisis stok V3..."):
        ...
        if penjualan_90.empty:
            ...
            st.session_state["stock_v3_result"] = None
            return
        ...
        **Multiplier Min Stock:**
        ...
        full = classify_stock_vs_so_statistik(
            full,
            city_col="City",
            kategori_col="Kategori Barang",
            status_col="Status Stock",
            stock_col="Stock Cabang",
            so_col="SO WMA",
        )
        ...
        # ── Add Stock (logika V2) berbasis kategori ─────────────────────────────────
        ...
        # ── Suggested PO (logika V2, 3 skenario) ──────────────────────────────────────
        ...
        st.session_state["stock_v3_result"]     = full.copy()
        st.session_state["stock_v3_bulan_cols"] = bulan_columns_renamed
        st.success("✅ Analisis Stok V3 berhasil dijalankan!")


# ── Render Hasil ───────────────────────────────────────────────────────────────
def _render_results_v3():
    result     = st.session_state["stock_v3_result"].copy()
    result     = result[result["City"] != "OTHERS"]
    bulan_cols = st.session_state.get("stock_v3_bulan_cols", [])
    KAT_COL    = "Kategori ABC (Log-Benchmark - WMA)"

    st.markdown("---")
    st.header("Filter Produk")
    col_f1, col_f2, col_f3 = st.columns(3)
    sel_kat   = col_f1.multiselect("Kategori:",    sorted(result["Kategori Barang"].dropna().unique().astype(str)), key="v3_kat")
    sel_brand = col_f2.multiselect("Brand:",       sorted(result["BRAND Barang"].dropna().unique().astype(str)),   key="v3_brand")
    sel_prod  = col_f3.multiselect("Nama Produk:", sorted(result["Nama Barang"].dropna().unique().astype(str)),    key="v3_prod")
    ...
    sel_abc    = col_h1.multiselect("Kategori ABC:", sorted(result[KAT_COL].dropna().unique().astype(str)), key="v3_abc")
    sel_status = col_h2.multiselect("Status Stock:", sorted(result["Status Stock"].dropna().unique().astype(str)), key="v3_status")
    ...
    with tab2:
        _render_pivot_v3(result, bulan_cols, KAT_COL)
    with tab3:
        _render_dashboard_v3(result, KAT_COL)

    # Download
    st.markdown("---")
    st.header("💾 Unduh Hasil Analisis Stock V3")
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        if "stock_v3_pivot_df" in st.session_state and not st.session_state["stock_v3_pivot_df"].empty:
            st.session_state["stock_v3_pivot_df"].to_excel(writer, sheet_name="All Cities Pivot", index=False)
    ...
    st.download_button(
        ...
        file_name=f"Hasil_Analisis_Stock_V3_{st.session_state.get('stock_v3_end', 'export')}.xlsx",
        ...
        with st.expander(f"📍 {city}"):
            ...
            for col in city_df.columns:
                if col in skip or not pd.api.types.is_numeric_dtype(city_df[col]):
                    continue
                fmt[col] = "{:.2f}" if any(x in col for x in ["Ratio", "Log", "Avg Log", "Rasio"]) else "{:.0f}"
            ...
            st.dataframe(
                ...
                .apply(lambda x: x.map(highlight_kategori_stok),    subset=["Kategori Stok"])
...
def _render_pivot_v3(result, bulan_cols, KAT_COL):
    ...
    with st.spinner("Membuat tabel pivot..."):
        ...
        # Hitung summary ALL V3
        summary_v3 = calculate_all_summary_v2(result)
        summary_v3.columns = [
            f"All_{c}" if c not in KEYS else c for c in summary_v3.columns
        ]
        ...
        pivot = pd.merge(pivot, summary_v3, on=KEYS, how="left")
        ...
        num_cols   = [c for c in df_style.columns if c not in KEYS
                      ...
                      and not any(x in c for x in ["Ratio", "Log", "Avg Log", "Rasio"])]
        float_cols = [c for c in df_style.columns if c not in KEYS
                      and any(x in c for x in ["Ratio", "Log", "Avg Log", "Rasio"])]
        ...
        for c in df_style.columns:
            if c in KEYS:
                continue
            if pd.api.types.is_numeric_dtype(df_style[c]) and not any(x in c for x in ["Ratio", "Log", "Avg Log", "Rasio"]):
                col_cfg[c] = st.column_config.NumberColumn(format="%.0f")
            elif any(x in c for x in ["Ratio", "Log", "Avg Log", "Rasio"]):
                col_cfg[c] = st.column_config.NumberColumn(format="%.2f")

        st.session_state["stock_v3_pivot_df"] = df_style.copy()
...
# ── Dashboard ──────────────────────────────────────────────────────────────────
def _render_dashboard_v3(result, KAT_COL):
    st.header("📈 Dashboard Analisis Stock V3")
    if result.empty:
    ...
    summary_v3 = calculate_all_summary_v2(result)
    if not summary_v3.empty and "Skenario_Distribusi" in summary_v3.columns:
        st.subheader("Distribusi Skenario per Produk")
        skenario_counts = summary_v3["Skenario_Distribusi"].value_counts()
    ...
    total_need  = summary_v3["All_Need_From_Supplier"].sum() if not summary_v3.empty else 0
    ...
    with col_t2:
        ...
        if not summary_v3.empty:
            top_sup = summary_v3.nlargest(5, "All_Need_From_Supplier")[
