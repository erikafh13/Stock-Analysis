...
# ==============================================================================
# LOGIKA V2 — Distribusi Proporsional dengan 4 Skenario + Lead Time
# ==============================================================================

LEAD_TIME_CABANG = 7   # hari pengiriman Surabaya → cabang
...
def calculate_add_stock_v2(df: pd.DataFrame, kategori_col: str,
    ...
    """
    Hitung Add Stock dengan buffer lead time 7 hari.

    Formula:
        Buffer      = CEIL(SO WMA / 30 × 7)
        Add Stock   = Max(0, Min Stock + Buffer - Stock Cabang)

    Kategori F → Add Stock = 0
    """
    mult        = df[kategori_col].map(DAYS_MULTIPLIER).fillna(1.0)
    min_stock   = np.where(
        (df[so_col] <= 0) | (df[kategori_col] == "F"),
    ...
    buffer      = np.where(
        df[kategori_col] == "F",
        0,
        np.ceil(df[so_col] / 30 * LEAD_TIME_CABANG),
    )
    add_stock   = np.where(
        df[kategori_col] == "F",
        0,
        np.maximum(0, min_stock + buffer - df[stock_col]),
    )
    return pd.Series(add_stock.astype(int), index=df.index)
